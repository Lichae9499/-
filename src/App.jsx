...
export default function ADHDTest() {
  ...
  const getHighlightedSymptoms = () => {
    ...
    questions.forEach(q => {
      ...
      // 选择了"经常"(3)或"总是"(4)的题目
      if (answer >= 3) {
        const frequencyLabel = answer === 4 ? '总是' : '经常';
        const frequencyColor = answer === 4 ? 'red' : 'orange';
        symptoms.push({
          text: q.text,
          id: q.id,
          section: q.section,
    ...
    return symptoms; // 返回所有高频症状，不限制数量
  };
                    ...
                    {highlightedSymptoms.map((symptom, idx) => (
                      ...
                      >
                        <span className={`flex-shrink-0 w-6 h-6 text-white rounded-full flex items-center justify-center text-xs font-bold ${
                          symptom.frequencyColor === 'red' ? 'bg-red-500' : 'bg-orange-500'
                        }`}>
                          {symptom.id}
                        </span>
                        <div className="flex-1">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <p className="text-gray-800 flex-1">{symptom.text}</p>
                            <span className={`flex-shrink-0 px-2 py-1 rounded-full text-xs font-bold ${
                              symptom.frequencyColor === 'red' 
                                ? 'bg-red-500 text-white' 
                                : 'bg-orange-500 text-white'
                            }`}>
                              {symptom.frequencyLabel}
                            </span>
                          </div>
